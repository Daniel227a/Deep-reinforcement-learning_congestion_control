...
def lstm_modeler(input_shape, num_units, output_shape):
    """
    Design a Long Short-Term Memory (LSTM) neural network.
    
    Args:
    - input_shape (tuple): Shape of the input data (e.g., (sequence_length, input_features)).
    - num_units (int): Number of LSTM units (i.e., the dimensionality of the LSTM's hidden state).
    - output_shape (int): Number of output units (e.g., for classification).
	...
	#num_units = é a quantidade de memória interna que a LSTM pode usar para lembrar informações passadas
	#input_shape  é a forma dos dados de entrada esperados pela camada LSTM tupla =(sequence_length, input_features)	
	#sequence_length=sequencia temporal, input_features =o número de entradas que a camada LSTM recebe em cada etapa de tempo da sequência.
	#(séries temporais univariadas ou  séries temporais multivariadas)
    model.add(tf.keras.layers.LSTM(num_units, input_shape=input_shape, return_sequences=True))


    
    # You can add more LSTM layers here if needed
    # Example: model.add(tf.keras.layers.LSTM(num_units, return_sequences=True))
